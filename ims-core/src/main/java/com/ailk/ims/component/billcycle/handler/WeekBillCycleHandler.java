...
public class WeekBillCycleHandler extends BillCycleHandler {

	public WeekBillCycleHandler(ChangeBillCycleContext billCycleContext,
			IMSContext context) {
	...
	public Date getFirstBillDate() {
    	...
    	// 是否大于请求日期
    	if (dayOfWeek >= firstBillDateDayIn) {
    		// 往后推一个礼拜
    		cal.add(Calendar.DATE, 7*billCycleContext.getBillCycle().getCycle_unit());
    	}
...
//        Date oldCycleEndDate = DateUtil.getFormatDate(oldCycleEnd, DateUtil.DATE_FORMAT_YYYYMMDD);
	...
	public void modifyShortBillCycleDelay() {
        ...
        */
        ...
        //计算expire_Date
        int dayOfWeek =getDayOfWeek(DateUtil.getDayOfWeek(oldCycleEndDate));
        Date shortPlanEndDate = DateUtil.offsetDate(oldCycleEndDate, Calendar.DATE, -dayOfWeek);
    	shortPlanEndDate = DateUtil.offsetDate(shortPlanEndDate, Calendar.DATE, newFirstBillDay);
        if(dayOfWeek >= newFirstBillDay){
        	shortPlanEndDate = DateUtil.offsetDate(shortPlanEndDate, Calendar.DATE, 7*newUnit);
        }
...
//    	cycleRun.setBillMonth(DateUtil.parserBillMonth(cycleRun.getCurrCycleEnd()));
    	...
    	imsLogger.dump("WeekHandler calVirtualCycleRun:",cycleRun);		
