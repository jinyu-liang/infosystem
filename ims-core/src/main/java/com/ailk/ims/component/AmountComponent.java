...
public class AmountComponent extends BaseComponent
{
     ...
     */
    public MeasureInfo parseBillingAmountMeasure(Integer measureId, Double amount, Long acctId)
    {
...
//            orgMeasureId = acct.getMeasureId();
        	...
        	targetMeasureId = queryMeasureIdByTypeAndModule(queryMeasureTypeId(acct.getMeasureId()), EnumCodeDefine.MODULE_BILLING);
//            return amount;
        }
        return new MeasureInfo( parseAmount(amount, orgMeasureId, targetMeasureId), targetMeasureId);
    }
    
    
    public Double parseBillingAmount(Integer measureId, Double amount, Long acctId)
    {
    	return parseBillingAmountMeasure(measureId,amount,acctId).getAmount();
    }
     ...
     * @Date 2012-10-11 yugb :Bug #60997 定购产品和设置二次议价
     */
    public MeasureInfo parseRatingAmountMeasure(Integer measureId, Double amount, Long acctId)
    {
        CaAccount acct = context.get3hTree().loadAcct3hBean(acctId).getAccount();
    ...
    {
    	return parseRatingAmountMeasure(measureId,amount,acctId).getAmount();
    }
