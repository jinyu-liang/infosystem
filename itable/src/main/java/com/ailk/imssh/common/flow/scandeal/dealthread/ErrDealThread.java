...
import com.ailk.imssh.common.flow.scandeal.dealprocess.ErrDealProcess;
 ...
 * @Date 2012-9-13
 */
public class ErrDealThread extends Thread
{
    ...
    public ErrDealThread(Queue queue, IBaseDeal realDeal, IConfig config)
    {
        this.queue = queue;
...
//                    {
                        ...
                        // 该用户无错单，执行业务操作
                        ErrDealProcess.dealProcess(dataHolder, baseDeal, config);
//                    }
                }
                else
                {
                    ...
                    imsLogger.debug("######ErrDealThread data is null sleep 1 s" );
                }
            }
            catch (Exception e)
            {
