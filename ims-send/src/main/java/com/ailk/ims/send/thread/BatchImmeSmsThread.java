...
import com.ailk.ims.send.service.ImmeSmsBatchSendService;
...
public class BatchImmeSmsThread extends Thread
{
    ...
    public BatchImmeSmsThread(SendConfig config,IImmeSmsSendService service,int subTableIndex){
        this.config=config;
    ...
    public void run()
    {
        //第一次启动判断短信的总量是不是超过某个值（100W），如果则需要按照手机号码的倒数第三位分10批处理
//        if (getSmsCount() > 1000000)
//        {
//            isBig = true;
//        }
    ...
    private long getSmsCount()
    {
        try
        {
            String tableName = "JD.TI_O_SMS_Batch_" + subTableIndex;
            return SendSpringUtil.getCommonDao().getClient().count(QB.create(TiOSmsBatch.class),tableName);
        }
