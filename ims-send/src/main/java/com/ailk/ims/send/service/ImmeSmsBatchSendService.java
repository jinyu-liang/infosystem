 ...
 */
public class ImmeSmsBatchSendService implements IImmeSmsSendService
{

	protected static Logger logger = Logger.getLogger(ImmeSmsBatchSendService.class);
    /**
     * 涉及两个数据源 <BR>
     * 返回发送给crm的数据条数 ， <BR>
     * 对于单表数据同步，统一采用该方法
    ...
    public int sendSingleTableData(SendConfig sendConfig, int subTableIndex, boolean isBig, int modIndex) throws BaseException
    {
        ...
        List<TiOSmsBatch> imsList = new ArrayList<TiOSmsBatch>();
        ...
        try
        {
            ...
            while(iter.hasNext())
            {
                ...
                imsList.add(IteratorCvt.immeSmsBatchCvt(immeBean));
        ...
        {
            Logger logger = Logger.getLogger(ImmeSmsBatchSendService.class);
     ...
     * @throws SQLException
     */
    private void immeSmsDeal(List<TiOSmsBatch> imsList, List<ROWID> rowidList, SendConfig sendConfig, int subTableIndex) throws SQLException
    {
        ...
        // insert到crm数据库
        List<com.ailk.openbilling.persistence.crmsms.entity.TiOSmsBatch> crmList = ImsCrmCvt.cvt2CrmImmeBatchList(imsList);
        ...
        List<TiOSmsBatchHis> imsHisList = HisObjCvt.cvt2HisBatchList(imsList, soDate);
...
//        String hisTableName = sendConfig.getHisTablePrefix() + subTableIndex+ "_" + SendUtil.getYYYYMM(soDate);
     ...
     */
    //TODO
    private List<TiOSmsBatch> authTiOSmsImme(List<TiOSmsBatch> list,int fetchSize)
    {
        List<TiOSmsBatch> authTiOSmsImmes = new ArrayList<TiOSmsBatch>();
        TemplateAuthService templateAuthService = SendSpringUtil.getTemplateAuthService();
        for (TiOSmsBatch imme : list)
        {
    ...
    private String printErrorSmsId(List<TiOSmsBatch> smsList)
    {
        if (smsList != null && !smsList.isEmpty())
        {
            StringBuffer sb = new StringBuffer("");
            sb.append("******** error SMS_NOTICE_ID list,[ ");
            for (TiOSmsBatch imme : smsList)
            {
    ...
    public String buildQuerySql(int subTableIndex)
    {
        ...
        .append(" t.sms_notice_id as smsNoticeId,t.partition_id as partitionId,t.eparchy_code as eparchyCode,t.brand_code as brandCode,t.in_mode_code as inModeCode," +
        		...
        		"t.revc4 as revc4,t.month as month,t.day as day from jd.ti_o_sms_batch_")//表字段 2013-07-23 liming15 新增字段validityPeriod
    ...
    {
        return new StringBuffer("delete from jd.ti_o_sms_batch_")
        .append(subTableIndex)
    ...
    public String buildQuerySql4Begin(int subTableIndex, int modIndex)
    {
        ...
        .append(" t.sms_notice_id as smsNoticeId,t.partition_id as partitionId,t.eparchy_code as eparchyCode,t.brand_code as brandCode,t.in_mode_code as inModeCode," +
                ...
                "t.refer_staff_id as referStaffId,t.refer_depart_id as referDepartId,t.deal_time as dealTime,t.deal_staffid as dealStaffid,t.deal_departid as dealDepartid," +
                "t.deal_state as dealState,t.done_code as doneCode,t.block_id as blockId,t.sms_code as smsCode,t.remark as remark,t.revc1 as revc1,t.revc2 as revc2,t.revc3 as revc3," +
                "t.revc4 as revc4,t.month as month,t.day as day from jd.ti_o_sms_batch_")//表字段
        .append(subTableIndex)
        .append(" t")
    ...
    private void dealORA00001(SendConfig sendConfig, int  subTableIndex, List<TiOSmsBatch> imsList, List<ROWID> rowidList)
    {
        try
        {   // 删除billing侧数据库中的相关数据，采用SQL拼装批量执行
            SendSpringUtil.getCommonDao().getClient().executeSqlBatch(buildDeleteSql(subTableIndex), getDeleteConds(rowidList));
